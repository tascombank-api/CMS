...
document.addEventListener('DOMContentLoaded', function() {
    function getAmount() {
        const amountElement = document.querySelector('.t706__cartwin-prodamount-price');
        if (amountElement) {
            return parseFloat(amountElement.textContent.replace(/[^0-9,.]/g, '').replace(',', '.'));
        }
        return 0; 
    }
    ...
    if (submitButton) {
        submitButton.addEventListener('click', function(event) {
            ...
            if (typeof plpb !== 'undefined' && typeof plpb.getButton === 'function') {
                const button1 = plpb.getButton({
                    buttonId: '507',
                    amount: amount, 
                    amountReadonly: false,
                    currency: 'UAH',
                    url: 'https://my-merchant.com',
                    host: 'inst2.paylink.com.ua',
                    telephone: false,
                    email: false,
                    contract: false,
                    fullname: false,
                ...
                const buttonUrl = button1.getUrl();
                if (buttonUrl) {
                ...
                console.error('Проблема с библиотекой plpb.');
            }
        });
    } else {
        console.error('.t-submit не найдена.');
    }
});
